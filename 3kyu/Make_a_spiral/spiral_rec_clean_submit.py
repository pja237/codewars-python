...
def spiral(arr, x, y, n):
    ...
    if not (x-1>=0 and arr[x-1][y]==0 and arr[x-2][y]==0):
        if can_right(arr,x,y): arr=spiral(arr,x,y+1,n)
    if can_down(arr,x,y): arr=spiral(arr,x+1,y,n)
    if can_left(arr,x,y): arr=spiral(arr,x,y-1,n)
    if can_up(arr,x,y): arr=spiral(arr,x-1,y,n)

    return arr

for i in spiralize(50):
    print i
